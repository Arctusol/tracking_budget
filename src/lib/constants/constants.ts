...
export const CATEGORY_IDS = {
  ...
  TOBACCO: 'b9c1d2e3-f456-7890-a123-456789abcdef',
  CREDITS: 'c9d1e2f3-4567-890a-b123-456789abcdef'
} as const;
...
export const CATEGORY_NAMES: { [key: string]: string } = {
  ...
  [CATEGORY_IDS.TOBACCO]: 'Tabac',
  [CATEGORY_IDS.CREDITS]: 'Crédits'
};
...
export const CATEGORY_HIERARCHY: { [key: string]: string[] } = {
  ...
  [CATEGORY_IDS.HEALTH]: [CATEGORY_IDS.MEDICAL, CATEGORY_IDS.PHARMACY, CATEGORY_IDS.VETERINARY, CATEGORY_IDS.INSURANCE, CATEGORY_IDS.PILL, CATEGORY_IDS.SUPPLEMENTS],
  [CATEGORY_IDS.SHOPPING]: [CATEGORY_IDS.CLOTHING,CATEGORY_IDS.GIFTS, CATEGORY_IDS.ELECTRONICS, CATEGORY_IDS.HOME, CATEGORY_IDS.BEAUTY, CATEGORY_IDS.JEWELRY],
  [CATEGORY_IDS.LEISURE]: [CATEGORY_IDS.ENTERTAINMENT, CATEGORY_IDS.SPORT, CATEGORY_IDS.BOOKS, CATEGORY_IDS.HOTEL, CATEGORY_IDS.TOBACCO],
  [CATEGORY_IDS.SERVICES]: [CATEGORY_IDS.SUBSCRIPTIONS],
};

export const CATEGORY_HIERARCHY_INCOME: { [key: string]: string[] } = {
  [CATEGORY_IDS.INCOME]: [CATEGORY_IDS.SALARY, CATEGORY_IDS.FREELANCE, CATEGORY_IDS.REIMBURSEMENTS, CATEGORY_IDS.CREDITS],
  [CATEGORY_IDS.TRANSFERS]: [CATEGORY_IDS.TRANSFER_ANTONIN || null, CATEGORY_IDS.TRANSFER_AMANDINE || null]
};

// Fonction utilitaire pour obtenir la catégorie parente des expenses et des incomes
export function getParentCategory(categoryId: string | null | undefined): string | null {
  if (!categoryId) return null;
  
  // Vérifier d'abord dans la hiérarchie des dépenses
  ...
  // Vérifier ensuite dans la hiérarchie des revenus
  for (const [parentId, children] of Object.entries(CATEGORY_HIERARCHY_INCOME)) {
    if (children.includes(categoryId)) {
      return parentId;
    }
  }
